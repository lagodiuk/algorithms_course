...
find_left_least( [First = #point{x=_, y=_} | T] ) ->
	find_left_least(T, First).
find_left_least( [], CurrLeftLeast = #point{x=_, y=_} ) ->
	CurrLeftLeast;
find_left_least(
	[#point{y = Y} = H | T], 
	#point{y = CurrY} ) when Y < CurrY ->
	find_left_least( T, H );
find_left_least(
        [#point{x = X, y = SameY} = H | T], 
        #point{x = CurrX, y = SameY} ) when X < CurrX ->
	find_left_least( T, H );
find_left_least( [_ | T], CurrLeftLeast ) ->
	find_left_least( T, CurrLeftLeast ).

remove_point( Points, PointToRemove = #point{x=_, y=_} ) ->
